...
function floodFill(startX, startY, replacementColor) {
    ...
    // Quick exit if same color
    if (targetColor === replacementColor) return;
    
    const queue = [[startX, startY]];
    const visited = new Set(); // To avoid processing same pixel multiple times
    
    while (queue.length > 0) {
        const [x, y] = queue.shift();
        const key = `${x},${y}`;
        
        // Skip if out of bounds, wrong color, or already visited
